...
def get_market_data():
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Fetch Nifty 50 data with increased timeout
            nifty = yf.Ticker("^NSEI")
            nifty_df = nifty.history(start=start_time, end=end_time, interval="1m", timeout=30)
            if not nifty_df.empty:
                plot_market_data(nifty_df, today.strftime('%Y-%m-%d'), 'NIFTY 50', 'nifty50_graph.png')
            
            # Fetch Sensex data with increased timeout
            sensex = yf.Ticker("^BSESN")
            sensex_df = sensex.history(start=start_time, end=end_time, interval="1m", timeout=30)
            ...
            print(f"Error generating market graphs (Attempt {attempt + 1}/{max_retries}): {str(e)}")
            if attempt < max_retries - 1:
                print(f"Retrying in {retry_delay} seconds...")
                time.sleep(retry_delay)
            else:
                print("Failed to generate market graphs after all retries")
                return False
...
def get_index_change(index_symbol, index_name):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"\nFetching {index_name} data (Attempt {attempt + 1}/{max_retries}):")
            ...
            print(f"Error fetching {index_name} data (Attempt {attempt + 1}/{max_retries}): {str(e)}")
            if attempt < max_retries - 1:
                print(f"Retrying in {retry_delay} seconds...")
                time.sleep(retry_delay)
            else:
...
def get_nifty_movers():
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Error fetching market movers data (Attempt {attempt + 1}/{max_retries}): {str(e)}")
            if attempt < max_retries - 1:
                print(f"Retrying in {retry_delay} seconds...")
                time.sleep(retry_delay)
            else:
...
def summarize_news(news_data):
    try:
        ...
        if not groq_api_key:
            print("Warning: GROQ_API_KEY not found. Skipping news summarization.")
            return "Market news summary unavailable at this time."
...
Powergrid Corp: showcased robust profits in Q3 ⚡"""
        ...
        print(f"Error in news summarization: {str(e)}")
        return "Market news summary unavailable at this time."
    ...
    try:
        ...
        if urls is None:
            urls = [
                ...
                "https://unlistedzone.com/shares/hindusthan-engineering-industries-limited-unlisted-shares",
                "https://unlistedzone.com/shares/national-commodity-derivatives-exchange-ncdex-limited-unlisted-shares",
